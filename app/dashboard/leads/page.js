...
export default function Leads() {
  ...
  async function loadLeads() {
    ...
    setStats({
      total: list.length,
      hot:   list.filter(l=>(l.ai_score||0)>=70).length,
      warm:  list.filter(l=>(l.ai_score||0)>=40&&(l.ai_score||0)<70).length,
      value: list.reduce((s,l)=>s+(l.estimated_value||0),0)
    })
    setLoading(false)
  }
  ...
  async function sendRecovery() {
    ...
    try {
      ...
      if (!session?.access_token) { toast.error("Session expired"); setSending(false); return }
      const phone = (selected.phone||selected.customers?.phone||"").replace(/[^0-9]/g,"")
      if (!phone) { toast.error("No phone number"); setSending(false); return }
      ...
      if (!result.success) { toast.error("Send failed: "+(result.error||"Unknown")); setSending(false); return }
      await supabase.from("leads").update({ status:"contacted", last_message_at:new Date().toISOString() }).eq("id",selected.id)
      setLeads(prev=>prev.map(l=>l.id===selected.id?{...l,status:"contacted"}:l))
      setSelected(s=>({...s,status:"contacted"}))
  ...
  const statusColor={open:acc,contacted:"#38bdf8",converted:"#a78bfa",lost:"#fb7185"}

  const filtered = leads.filter(l=>{
    const name=(l.customers?.name||l.name||l.phone||"").toLowerCase()
    const matchSearch=name.includes(search.toLowerCase())||(l.last_message||"").toLowerCase().includes(search.toLowerCase())
    const matchFilter=filter==="all"||l.status===filter||(filter==="hot"&&(l.ai_score||0)>=70)
        ...
        <div className="main">
          ...
          <div className="content">
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:11}}>
              {[
                {l:"Total Leads",   v:stats.total, c:tx},
                {l:"🔥 Hot Leads",  v:stats.hot,   c:"#f59e0b"},
                {l:"♨ Warm Leads", v:stats.warm,  c:"#38bdf8"},
                {l:"Pipeline Value",v:`₹${stats.value.toLocaleString()}`, c:acc},
              ].map(s=>(
              ...
              <div style={{background:card,border:`1px solid ${cbdr}`,borderRadius:13,overflow:"hidden"}}>
                <div style={{padding:"10px 14px",borderBottom:`1px solid ${bdr}`,display:"flex",flexDirection:"column",gap:8}}>
                  ...
                  <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                    {["all","open","hot","contacted","converted","lost"].map(f=>(
                      <button key={f} onClick={()=>setFilter(f)} style={{padding:"3px 10px",borderRadius:100,fontSize:11,fontWeight:600,cursor:"pointer",border:`1px solid ${filter===f?acc+"55":cbdr}`,background:filter===f?acc+"18":"transparent",color:filter===f?acc:txm,fontFamily:"'Plus Jakarta Sans',sans-serif",textTransform:"capitalize"}}>
                        {f}
                      </button>
                    ))}
                  </div>
                ...
                <div style={{overflowY:"auto",maxHeight:"calc(100vh - 300px)"}}>
                  {loading?(
                    <div style={{textAlign:"center",padding:"40px 0",color:txf,fontSize:13}}>Loading leads…</div>
                  ):filtered.length===0?(
                    <div style={{textAlign:"center",padding:"40px 16px",color:txf}}>
                      <div style={{fontSize:28,opacity:0.3,marginBottom:8}}>◉</div>
                      <div style={{fontSize:13,fontWeight:600}}>No {filter} leads</div>
                      <div style={{fontSize:11,marginTop:4}}>Leads are captured automatically from WhatsApp</div>
                    </div>
                  ):filtered.map(lead=>{
                    const name=lead.customers?.name||lead.name||lead.phone||"Unknown"
                    const phone=lead.phone||lead.customers?.phone||""
                    const score=lead.ai_score||0
                    const isSel=selected?.id===lead.id
                    return(
                      <div key={lead.id} onClick={()=>setSelected(isSel?null:lead)}
                        style={{padding:"11px 14px",borderBottom:`1px solid ${bdr}`,cursor:"pointer",background:isSel?adim:"transparent",borderLeft:`3px solid ${isSel?acc:"transparent"}`,transition:"all 0.1s"}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                          <span style={{fontWeight:700,fontSize:13,color:tx}}>{name}</span>
                          <div style={{display:"flex",gap:5,alignItems:"center"}}>
                            <span style={{fontSize:10,fontWeight:700,color:scoreColor(score),background:scoreColor(score)+"15",border:`1px solid ${scoreColor(score)}33`,borderRadius:100,padding:"1px 7px"}}>{scoreLabel(score)}</span>
                            <span style={{fontSize:10,fontWeight:700,color:statusColor[lead.status]||txm,background:(statusColor[lead.status]||txm)+"15",border:`1px solid ${(statusColor[lead.status]||txm)}33`,borderRadius:100,padding:"1px 7px",textTransform:"capitalize"}}>{lead.status||"open"}</span>
                          </div>
                        </div>
                        <div style={{fontSize:12,color:txm,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",marginBottom:3}}>{lead.last_message||"No message"}</div>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                          <span style={{fontSize:11,color:txf}}>{phone}</span>
                ...
                {selected?(
                  <>
                    <div style={{fontWeight:700,fontSize:14,color:tx,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      Lead Details
                      <button onClick={()=>setSelected(null)} style={{background:"transparent",border:"none",color:txf,cursor:"pointer",fontSize:18}}>×</button>
                    </div>
                    {[
                      ["Name",   selected.customers?.name||selected.name||"—"],
                      ...
                      ["Last contact", selected.last_message_at?new Date(selected.last_message_at).toLocaleDateString("en-IN",{day:"numeric",month:"short"}):"—"],
                    ].map(([l,v])=>(
                      <div key={l} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:`1px solid ${bdr}`}}>
                        ...
                        <span style={{fontSize:12,fontWeight:600,color:tx,textTransform:l==="Status"?"capitalize":"none"}}>{v}</span>
                      </div>
                    ))}
                    {selected.last_message&&(
                      <div style={{background:ibg,border:`1px solid ${cbdr}`,borderRadius:8,padding:"10px 12px",fontSize:12.5,color:txm,lineHeight:1.6}}>
                        "{selected.last_message}"
                      </div>
                    )}
                    <div>
                      <div style={{fontSize:12,color:txm,fontWeight:600,marginBottom:6}}>Send Recovery Message</div>
                      <textarea value={msg} onChange={e=>setMsg(e.target.value)}
                        placeholder="Hi! Just checking in — would you like to book an appointment? 😊"
                        rows={3} style={{background:ibg,border:`1px solid ${cbdr}`,borderRadius:8,padding:"9px 12px",color:tx,fontSize:13,outline:"none",width:"100%",fontFamily:"'Plus Jakarta Sans',sans-serif",resize:"vertical",lineHeight:1.6,marginBottom:8}}/>
                      <button onClick={sendRecovery} disabled={sending||!msg.trim()}
                        style={{width:"100%",padding:"9px",borderRadius:8,background:(!msg.trim()||sending)?ibg:acc,border:"none",color:(!msg.trim()||sending)?txm:"#000",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"'Plus Jakarta Sans',sans-serif",opacity:sending?0.6:1}}>
                      ...
                      {["contacted","converted","lost"].map(s=>(
                        selected.status!==s&&(
                          <button key={s} onClick={()=>updateStatus(selected.id,s)}
                            style={{flex:1,padding:"7px",borderRadius:7,background:ibg,border:`1px solid ${cbdr}`,color:txm,fontSize:11,cursor:"pointer",fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:600,textTransform:"capitalize"}}>
                            → {s}
                          </button>
                        )
                      ))}
                  ...
                  <div style={{textAlign:"center",padding:"40px 16px",color:txf}}>
                    ...
                    <div style={{fontSize:12,lineHeight:1.7}}>Click any lead<br/>to see details & send recovery</div>
                  </div>
                )}
              </div>
            </div>
